...
class LeaderboardEvaluator(object):
    ...
    def __init__(self, args, statistics_manager):
        ...
        print("\033[1m> Setting up the CARLA server\033[0m")
        print("Host: {}".format(args.host))
        print("Port: {}".format(args.port))

        self.client = carla.Client(args.host, int(args.port))

        print("\033[1m> Setting synchronous mode\033[0m")
        if args.timeout:
            self.client_timeout = float(args.timeout)
        
        print("\033[1m> Setting client timeout to {}s\033[0m".format(self.client_timeout))
        self.client.set_timeout(self.client_timeout)

        print("\033[1m> Setting up the TrafficManager with PORT {}\033[0m".format(args.trafficManagerPort))
        self.traffic_manager = self.client.get_trafficmanager(int(args.trafficManagerPort))

        print("Waiting for CARLA world to be ready ({}s)".format(self.wait_for_world))
        dist = pkg_resources.get_distribution("carla")
        if dist.version != 'leaderboard':
            if LooseVersion(dist.version) < LooseVersion('0.9.10'):
                raise ImportError("CARLA version 0.9.10.1 or newer required. CARLA version found: {}".format(dist))

        print("\033[1m> CARLA Version: {}\033[0m".format(dist.version))
        # Load agent
        module_name = os.path.basename(args.agent).split('.')[0]
        sys.path.insert(0, os.path.dirname(args.agent))

        print("\033[1m> Loading agent from {}\033[0m".format(args.agent))

        self.module_agent = importlib.import_module(module_name)

        print("\033[1m> Setting up the ScenarioManager\033[0m")
        ...
        print("\033[1m> Setting up the agent watchdog\033[0m")
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        print("Agent: {}".format(self.module_agent))
    ...
    def run(self, args):
        ...
        print("\n")
        print("\033[1m> Starting the route execution\033[0m")
        print("Routes: {}".format(args.routes))
        print("Scenarios: {}".format(args.scenarios))
        print("Repetitions: {}".format(args.repetitions))
        ...
        while route_indexer.peek():
            # setup
            print("\n\033[1m========= Setting up the route =========\033[0m")
            config = route_indexer.next()

            print("\n\033[1m========= Running {} (repetition {}) =========".format(config.name, config.repetition_index))
            # run
...
def main():
    ...
    try:
        print("\033[1m> Setting up the leaderboard evaluator\033[0m")
        leaderboard_evaluator = LeaderboardEvaluator(arguments, statistics_manager)
        print("\033[1m> Running the leaderboard evaluator\033[0m")
...
if __name__ == '__main__':
    print("Running Leaderboard Evaluator")
    main()
